...
# Path to the main data filestreamlit run app/dashboard.py
from pathlib import Path
BASE_DIR = Path(__file__).resolve().parent
DEFAULT_FILE = BASE_DIR.parent / "data" / "Fuel.xlsx"
try:
...
FUEL_DB_FILE = DATA_DIR / "Fuel.xlsx"
LOGO_FILE = BASE_DIR.parent / "tonga-energy-logo.jpg"
BACKGROUND_FILE = BASE_DIR.parent / "background.jpg"
...
# Add missing PRICE_FILE definition
PRICE_FILE = BASE_DIR.parent / "data" / "Transformed_for_Analysis.xlsx"
...
@st.cache_data
def load_price_data(mtime=None):
    file_path = FUEL_DB_FILE
...
def save_actual_data(file_path, date, company, location, fuel_type, closing_stock, offtake, tonga_power_offtake=0, allow_update=False):
    ...
    try:
        ...
        if existing_row is not None:
            ...
            ws[f"B{existing_row}"] = closing_stock
            ws[f"C{existing_row}"] = offtake
        ...
        # Add data — column order: Date, Closing Stock, Offtake, Fuel Type, Company, Location, Tonga Power Offtake
        ws[f"A{next_row}"] = date
        ws[f"B{next_row}"] = closing_stock
        ws[f"C{next_row}"] = offtake
        ws[f"D{next_row}"] = fuel_type
        ws[f"E{next_row}"] = company
        ws[f"F{next_row}"] = location
...
def save_resupply_data(file_path, date, company, location, fuel_type, quantity, allow_update=False):
    ...
    try:
        ...
        # Resupply column order: Date=1, Quantity=2, Fuel Type=3, Company=4, Location=5
        existing_row = _sheet_find_entry_row(
            ws, date, company, location, fuel_type,
            date_col=1, company_col=4, location_col=5, fuel_col=3
        )
        if existing_row is not None:
            if not allow_update:
                return (
        ...
        # Add data — column order: Date, Quantity, Fuel Type, Company, Location
        ws[f"A{next_row}"] = date
        ws[f"B{next_row}"] = quantity
        ws[f"C{next_row}"] = fuel_type
        ws[f"D{next_row}"] = company
        ws[f"E{next_row}"] = location
...
def save_fuel_price(file_path, date, fuel_type, price_type, price):
    ...
    try:
        ...
        # Add data — column order: Date, Price, Price_Type, Fuel
        ws[f"A{next_row}"] = date
        ws[f"B{next_row}"] = price
...
def save_tariff(file_path, date, component, period, value):
    ...
    try:
        ...
        # Add data — column order: Month, Value, Year, Component
        ws[f"A{next_row}"] = date
        ws[f"B{next_row}"] = value
        ws[f"C{next_row}"] = period
        ws[f"D{next_row}"] = component
...
st.markdown(
    (
        "<div style='padding: 0.9rem 0.62rem; margin: 3.5rem 0 0.24rem 0; border: 1.6px solid rgba(125, 211, 252, 0.72); border-radius: 12px; "
        "background: linear-gradient(180deg, rgba(30, 64, 175, 0.12), rgba(14, 22, 32, 0.66)); box-shadow: 0 0 0 1px rgba(186, 230, 253, 0.2), 0 0 10px rgba(56, 189, 248, 0.16); backdrop-filter: blur(2px);'>"
        "<div style='display:flex; align-items:center; gap:0.25rem;'>"
        f"{logo_html}"
        "<div>"
        "<h1 style='margin: 0; font-size: 2.2rem; line-height: 1.2; font-weight: 700; letter-spacing: 0.2px; color: #F2F6FA;'>Tonga National Fuel Supply and Consumption Dashboard</h1>"
        "<p style='margin: 0.18rem 0 0 0; opacity: 0.88; font-size: 1.08rem; line-height: 1.3; color: #E0ECFA;'>Staged Energy and Fuel Supply Plan Monitoring</p>"
        "</div>"
        "</div>"
        "</div>"
    ),
...
try:
    ...
    sheets, actual_df, resupply_df, terminal_df = load_data(file_mtime=file_mtime)
...
try:
    price_df, tariff_df = load_price_data(mtime=FUEL_DB_FILE.stat().st_mtime)
except Exception as _price_exc:
    st.warning(f"Could not load price/tariff data: {_price_exc}")
...
def build_summary_pdf_bytes():
    ...
    try:
        ...
        y -= 14
        pdf.setFont("Helvetica", 9)
        # Total Delivered = resupply entries with Date <= today
        today = pd.Timestamp.now().normalize()
        if not filtered_resupply.empty and "Date" in filtered_resupply.columns and "Quantity" in filtered_resupply.columns:
            delivered_data = filtered_resupply[pd.to_datetime(filtered_resupply["Date"], errors="coerce") <= today]
            total_delivered = delivered_data["Quantity"].sum()
        else:
            total_delivered = 0.0
        pdf.drawString(28, y, f"Total Fuel: {total_stock:,.0f} L")
        pdf.drawString(200, y, f"Total Delivered: {total_delivered:,.0f} L")
        pdf.drawString(390, y, f"Upcoming Supply: {upcoming_supply:,.0f} L")
        y -= 12
        pdf.drawString(28, y, f"Total Offtake: {total_consumption:,.0f} L")
        pdf.drawString(390, y, f"Days of Cover: {days_of_cover:.2f} ({cover_status})")
        ...
        if not _imp_base.empty and "Closing Stock" in _imp_base.columns:
            ...
            if not imp_df.empty:
                ...
                tbl_w = page_w - 48
                margin_l = 24
                n_fuel = len(all_fuel_cols)
                date_w = 105
                total_w = 90
                fuel_w = (tbl_w - date_w - total_w) / max(n_fuel, 1)
                col_headers = ["Date"] + all_fuel_cols + ["Total (L)"]
                col_x = [margin_l]
                col_x += [margin_l + date_w + i * fuel_w for i in range(n_fuel)]
                col_x += [margin_l + date_w + n_fuel * fuel_w]

                def draw_imp_header(pdf, y):
                    pdf.setFillColorRGB(0.18, 0.35, 0.55)
                    pdf.rect(margin_l, y - row_h + 3, tbl_w, row_h, fill=1, stroke=0)
                    pdf.setFillColorRGB(1, 1, 1)
                    pdf.setFont("Helvetica-Bold", 8)
                    pdf.drawString(col_x[0] + 3, y - 8, col_headers[0])
                    for i, hdr in enumerate(col_headers[1:], 1):
                        w = fuel_w if i < len(col_headers) - 1 else total_w
                        pdf.drawRightString(col_x[i] + w - 3, y - 8, hdr)
                ...
                pdf.setFillColorRGB(1, 1, 1)
                pdf.setFont("Helvetica-Bold", 8)
                pdf.drawString(col_x[0] + 3, y - 8, "Grand Total:")
                for i, fc in enumerate(all_fuel_cols):
                    fc_grand = imp_df[imp_df["Fuel Type"] == fc]["_import"].sum()
                    pdf.drawRightString(col_x[1 + i] + fuel_w - 3, y - 8, f"{fc_grand:,.0f}")
                pdf.drawRightString(col_x[-1] + total_w - 3, y - 8, f"{grand_total:,.0f} L")
                pdf.setFillColorRGB(0, 0, 0)
            else:
                pdf.setFont("Helvetica", 9)
        ...
        # --- TPL Offtake table (still on Page 2, or new page if needed) ---
        tpl_base = actual_df.copy() if not actual_df.empty else filtered_actual.copy()
        if "Date" in tpl_base.columns:
            tpl_base["Date"] = pd.to_datetime(tpl_base["Date"], errors="coerce")
        if "Tonga Power Offtake" in tpl_base.columns:
            tpl_data = tpl_base[
                tpl_base["Tonga Power Offtake"].notna()
                & (tpl_base["Tonga Power Offtake"] > 0)
                & (tpl_base["Date"] <= today)
            ].copy()
        ...
        if not filtered_resupply.empty and "Quantity" in filtered_resupply.columns:
            delivery_df2 = filtered_resupply.copy()
            delivery_df2["Date"] = pd.to_datetime(delivery_df2["Date"], errors="coerce")
            future_deliveries = delivery_df2[delivery_df2["Date"] > today].dropna(subset=["Quantity"]).sort_values("Date")

            if not future_deliveries.empty:
                # Table header
                col_x = [24, 130, 300, 450, 620]
                ...
                for i, header in enumerate(col_headers):
                    pdf.drawString(col_x[i] + 3, y - 8, header)
                pdf.setFillColorRGB(0, 0, 0)
                y -= row_h + 2

                pdf.setFont("Helvetica", 9)
                for row_idx, (_, row) in enumerate(future_deliveries.iterrows()):
                    if y < 40:
                        break
                    if row_idx % 2 == 0:
                        pdf.setFillColorRGB(0.94, 0.97, 1.0)
                        pdf.rect(24, y - row_h + 3, page_w - 48, row_h, fill=1, stroke=0)
                        pdf.setFillColorRGB(0, 0, 0)
                    date_val = row["Date"].strftime("%d %b %Y") if pd.notna(row.get("Date")) else ""
                    company_val = str(row.get("Company", ""))
                    fuel_val = str(row.get("Fuel Type", ""))
                    location_val = str(row.get("Location", ""))
                    qty_val = f"{float(row['Quantity']):,.0f}" if pd.notna(row.get("Quantity")) else ""
                    for i, val in enumerate([date_val, company_val, fuel_val, location_val, qty_val]):
                        pdf.drawString(col_x[i] + 3, y - 8, val)
                    y -= row_h

                # Totals footer
                y -= 4
                total_future = future_deliveries["Quantity"].sum()
                pdf.setFillColorRGB(0.18, 0.35, 0.55)
                pdf.rect(24, y - row_h + 3, page_w - 48, row_h, fill=1, stroke=0)
                pdf.setFillColorRGB(1, 1, 1)
                pdf.setFont("Helvetica-Bold", 9)
                pdf.drawString(col_x[0] + 3, y - 8, "Total Scheduled:")
                pdf.drawString(col_x[4] + 3, y - 8, f"{total_future:,.0f} L")
                pdf.setFillColorRGB(0, 0, 0)
            else:
                pdf.setFont("Helvetica", 9)
                pdf.drawString(24, y, "No upcoming deliveries scheduled.")
        else:
            pdf.setFont("Helvetica", 9)
            pdf.drawString(24, y, "No resupply data available.")
...
if active_section == "📊 Fuel Supply":
    ...
    if terminal_df.empty:
        st.info("No terminal capacity data available. Use the Data Entry tab to add terminal records.")
    else:
        # Terminal data filters
        t_locations = sorted([x for x in terminal_df["Location"].dropna().unique()]) if "Location" in terminal_df.columns else []
        t_info = sorted([x for x in terminal_df["Terminal Info"].dropna().unique()]) if "Terminal Info" in terminal_df.columns else []
    ...
    if price_df is None or tariff_df is None:
        st.warning(
            "Price & tariff data could not be loaded from Fuel.xlsx. "
            "Check that FuelPrice_Long and Tariff_Long sheets exist."
        )
    else:
        # ── Fuel Prices ───────────────────────────────────────────────
        ...
        if not fp_filtered.empty:
            ...
            fig_fp.update_xaxes(tickformat="%Y", dtick="M12")
        ...
        if not tr_filtered.empty:
            ...
            fig_tr.update_xaxes(tickformat="%Y", dtick="M12")
    ...
    if not check_session_timeout():
        ...
        with entry_tabs[1]:
            ...
            with st.form("resupply_form", border=True):
                ...
                # Removed duplicate and broken resupply entry block using undefined variables
...
with pdf_col1:
    if st.button("⬇️ Generate Summary PDF", key="generate_pdf_btn"):
        if not REPORTLAB_AVAILABLE:
            ...
            result = build_summary_pdf_bytes()
            if result is None:
                st.error("PDF generation failed. Check the terminal for errors.")
            else:
                st.session_state.summary_pdf_bytes = result
...
st.markdown(
    (
        "<div style='margin-top: 0.6rem; padding: 0.52rem 0.75rem; background: rgba(229, 231, 235, 0.95); "
        "border: 1px solid rgba(209, 213, 219, 0.95); color: #4B5563; font-size: 0.88rem; line-height: 1.35; display: flex; "
        "justify-content: center; align-items: center; gap: 0.7rem; flex-wrap: wrap; text-align: center; border-radius: 12px;'>"
        f"{footer_logo_html}"
        f"<span>Developed by Department of Energy under Ministry of MEIDECC for Staged Energy and Fuel Supply Plan Monitoring | Data updated: {last_sync}</span>"
        "</div>"
    ),
    unsafe_allow_html=True
)
